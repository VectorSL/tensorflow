...
// ----------------------------------------------------------------------------------------
    ...
    public:
        ...
        shape_predictor train (
            ...
            const std::vector<std::vector<full_object_detection> >& objects,std::vector<std::string>& imgnamevec
        ) const
        {
            using namespace impl;
           /* DLIB_CASSERT(images.size() == objects.size() && images.size() > 0,
			...
			//并行parallel_for_
			//class Read_Img_Paralle : public ParallelLoopBody
			//{
			//public:
			//	Read_Img_Paralle(int _stripSize,int _maxnum,std::vector<training_sample> &_samplesIn,std::vector<training_sample> &_samplesOut,std::vector<std::string> _imagenamevec,matrix<float,0,1> _initial_shape,
			//		 std::vector<unsigned long> _anchor_idx,std::vector<dlib::vector<float,2> > _deltas,Mutex* _mtx)
			//	{
			//		samplesIn=_samplesIn;
			//		samplesOut=&_samplesOut;
			//		strip_Size=_stripSize;
			//		max_num=_maxnum;
			//		imagenamevec=_imagenamevec;
			//		initial_shape=_initial_shape;
			//		anchor_idx=_anchor_idx;
			//		deltas=_deltas;
			//		mtx = _mtx;///!!!!重要，互斥锁,锁住之后在pushback
			//	}
			//	void operator()(const Range& range) const
			//	{
			//		int y1 = range.start * strip_Size;
			//		int y2 = cv::min(range.end * strip_Size, max_num);
			...
			//			training_sample sam;
			//			sam=samplesIn[y];
			//			//cout<<imagenamevec[sam.image_idx]<<endl;
			//		    load_image(img, imagenamevec[sam.image_idx]);
			//		    extract_feature_pixel_values(img, sam.rect,
   //                       sam.current_shape, initial_shape, anchor_idx,
   //                       deltas, sam.feature_pixel_values);
			//			mtx->lock();
			//			samplesOut->push_back(sam);
			//			mtx->unlock();
			//			//cout<<samplesOut->size()<<endl;
			//		}
			//	}
			//	std::vector<training_sample> samplesIn;
			//	std::vector<training_sample> *samplesOut;
			//	std::vector<std::string> imagenamevec;
			//	matrix<float,0,1> initial_shape;
			//	std::vector<unsigned long> anchor_idx; 
   //             std::vector<dlib::vector<float,2> > deltas;
			//	int strip_Size,max_num;
			//	Mutex* mtx;
			//};

            for (unsigned long cascade = 0; cascade < get_cascade_depth(); ++cascade)
            {
				cout<<"cascade level: "<<cascade<<"/"<<get_cascade_depth()<<endl;
                // Each cascade uses a different set of pixels for its features.  We compute
				...
				try{
					...
					{
						//if(i>1 && samples[i].image_idx==samples[i-1].image_idx )
						//{
						//}
						//else
						//{
						//  //cout<<imgnamevec[samples[i].image_idx]<<endl;
						//  load_image(img, imgnamevec[samples[i].image_idx]);
						//}
						...
						//extract_feature_pixel_values(img, samples[i].rect,
						//	samples[i].current_shape, initial_shape, anchor_idx,
						//	deltas, samples[i].feature_pixel_values);
					}
				}
				...
				//std::vector<training_sample> samples_par;
				//samples_par.clear();
				//std::vector<training_sample> samples_loc;
				//samples_loc.clear();
				//Mutex mtx;
				//int stripCount=100;
				//int stripSize=samples.size()/stripCount+1;
				//parallel_for_(Range(0, stripCount),Read_Img_Paralle(stripSize,samples.size(),samples,samples_loc,imgnamevec,initial_shape,anchor_idx,deltas,&mtx));			
				//samples_par.insert(samples_par.end(),samples_loc.begin(),samples_loc.end());
				//samples.clear();
				//samples=samples_par;
    ...
    private:
        ...
        {
            samples.clear();
			std::cout<<"samples Max_size: "<<samples.max_size()<<std::endl;
            ...
            for (unsigned long i = 0; i < objects.size(); ++i)
            {
                for (unsigned long j = 0; j < objects[i].size(); ++j)
                {
					...
					//std::cout<<"samples capacity: "<<samples.capacity()<<std::endl;
